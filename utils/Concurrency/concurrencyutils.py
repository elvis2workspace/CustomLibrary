...
# 该函数将一系列给定的任务分配给对应的处理函数(分配顺序不确定)
import datetime
...
def do_threaded_work(work_items, work_func, num_threads=None, per_sync_timeout=1, preserve_result_ordering=True):
    """Executes work_func on each work_item.
        Note:Execution order is not preserved,but output ordering is(optionally).
        Parameters:
        ...
        Return:
        ---list of results from applying work_func to each work_item.
        Order is op 将线程id写入所有日志消息tionally preserved.
...
# 该类创建一个线程，它将从一个同步的工作队列中拉取工作任务并将处理结果写入同步结果队列
class ThreadedWorker(threading.Thread):
    """Generic Threaded Worker
    Input towork_func:item from work_queue
    Example usage:
    import Queue
    urls_to_process=[&quot;http://url1.com", "http://url2.com", "http://site1.com", "http://site2.com"]
    work_queue=Queue.Queue()
...
class AsyncZip(threading.Thread):
    ...
    def run(self):
        ...
        print 'Finished backgroup zip of: ', self.infile


if __name__ == '__main__':
    background = AsyncZip('concurrencyutils.py', 'myarchive.zip')
    background.start()
    print 'The main program continues to run in foreground.'
    background.join()
    print  'Main program waited until backgound was done.'
