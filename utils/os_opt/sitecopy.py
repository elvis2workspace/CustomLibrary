# Python语言: sitecopy,一个用于山寨网站UI的脚本
# coding:utf-8
import os
import re
import shutil
...
class Fetcher:
    ...
    def __init__(self, timeout=10, threads=None, stacksize=32768 * 16, loginfunc=None):
        ...
        self.opener = urllib.request.build_opener(cookie_support, encoding_support, urllib.request.HTTPHandler)
        self.req = urllib.request.Request('http://www.hsbc.com')
...
class SiteCopyer:
    ...
    def copy(self):
        ...
        links.extend(re.compile(r'<script[^>]*res=(.*?)[ >]', re.I).findall(page))
        links.extend(re.compile(r'<img[^>]*res=(.*?)[ >]', re.I).findall(page))
        templinks = []
        for link in links:
