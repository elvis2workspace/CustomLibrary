...
def ssh_mysql_dump(ip, port, db_name, tbl_name, dump_name, t1, t2):
    try:
        ...
        mysqldump_cmd = 'mysqldump -uroot %s %s -w "t_when > from_unixtime(%d) and t_when < from_unixtime(%d)" > %s' % (
...
def mysqldump_to_csv(dump_name, csv_name):
    try:
        mysqldump_to_csv_cmd = 'cd ~/download/mysqldump-to-csv/ && python mysqldump_to_csv.py %s > %s' % (
        dump_name, csv_name)
